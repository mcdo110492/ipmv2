(function ()
{
    ...
    /** @ngInject */
    function EmploymentStatusController($scope,api,ModalConfig,$q,toaster)
    {
        var bookmark;
        var vm      =   this;
            vm.query = {
            ...
            function getLists()
            {
                vm.promise = api.employmentStatus.employmentStatus.get(vm.query,success).$promise;
                function success (r)
                {
                    vm.lists = r;
                }
            ...
            function add(ev)
            {
                var tempUrl     = 'app/main/employment-status/dialog/employment-status-dialog.html',
                    localData   = {data:null},
                    ctrl        = 'EmploymentStatusDialogController',
                    ctrlAs      = 'dcvm';
            ...
            function checkData(newVal,oldVal,field)
            {
                ...
                else if(newVal != oldVal)
                {
                    ...
                    api.employmentStatus.check.save(datastring,success);
                    function success(r)
                    {
                        if(r.status == 403)
                        {
            ...
            function updateData(data,id)
            {
                    ...
                    var datastring = {employment_status_id:id,employment_status:data.employment_status};
                    api.employmentStatus.employmentStatus.update(datastring,success);
